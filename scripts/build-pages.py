...
for slug,(name,color) in PRODUCTS.items():
 ...
 desc='<p class="description">'+{'expenses':'Your spending. One clear picture.','aura':'Your stars. A clearer view.','lumen':'Small quizzes. Big sparks.'}[slug]+'</p>'
 stores=re.search(r'<div class="store-links"[\s\S]*?</div>',s).group(0)
 note=re.search(r'<p class="release-note">.*?</p>',s).group(0)
 get_description=f'Get {name} by gLitCh Labs. '+('Available on Google Play and the App Store.' if slug=='expenses' else 'Download availability and beta links.')
 getdir=ROOT/slug/'get';getdir.mkdir(exist_ok=True)
 getdir.joinpath('index.html').write_text(f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="{color}"><meta name="description" content="{get_description}"><title>Get {name} — gLitCh Labs</title><link rel="icon" href="../../assets/{slug}.png">{styles('../../')}<link rel="stylesheet" href="../../get.css?v=compact-2"></head><body class="product-site get-site {slug}"><header class="navigation"><a class="brand" href="../index.html"><img src="../../assets/{slug}.png" alt="" width="32" height="32"><span class="product-name">{name}<span class="name-dot" aria-hidden="true">.</span></span></a><nav aria-label="Main navigation"><a href="../support.html">Support</a><a href="../../index.html#{slug}">All apps ↗</a></nav></header><main class="product-landing"><div class="landing-visual"><img class="landing-icon" src="../../assets/{slug}.png" alt="" width="1280" height="1280"></div><div class="landing-copy">{title}{desc}{stores}{note}</div></main><footer class="product-footer"><a href="../../index.html">gLitCh Labs</a><nav aria-label="Product resources">{resources(slug,'../')}</nav></footer></body></html>''')
p=ROOT/'index.html';s=p.read_text()
for sheet in ('fonts','typography','navigation'):
 if not re.search(r'href="'+re.escape(sheet)+r'\.css(?:\?[^"]*)?"',s):s=s.replace('</head>',f'<link rel="stylesheet" href="{sheet}.css"></head>')
